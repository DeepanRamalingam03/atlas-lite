...
class FakeOpenAIClient(BaseClient):
    ...
    def generate(self, prompt: str) -> str:
        ...
        return "Manager response with project awareness."


test_root = Path(".atlas_project_scanner_test")
...
assert "core/assistant_manager.py" in manager.project_context
assert "atlas_core/constitution/VISION.md" in manager.project_context
...
assert "core/assistant_manager.py" in gemini.last_prompt
...
assert "atlas_core/constitution/VISION.md" in openai.prompts[-1]

shutil.rmtree(test_root)

print("Project scanner integration passed")
